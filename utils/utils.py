...
class CustomFormatter(Formatter):
    def __init__(self, args=None, fmt=None, datefmt=None, style='%'):
        super().__init__(fmt, datefmt, style)
        self.args = args
    
    def format(self, record):
        if self.args is not None:
            record.log_path = self.args.mod
        return super().format(record)
    
    
def create_logger(args, log_level=logging.INFO, filemode='w', name='DV-3DLane'):
    ...
    fmt = '%(asctime)s - %(log_path)s - %(levelname)s - %(message)s'
    formatter = CustomFormatter(args, fmt)
...
def define_args():
    ...
    parser.add_argument('--print_freq', type=int, default=50, help='padding')
    ...
    # output 2d results during val
    parser.add_argument('--pred_2d', default=False, action='store_true', 
                        help='whether output 2d pred results & dump them during validation.')
...
def remove_outlier(lane, max_dx=5, max_dz=5):
    if lane.shape[0] < 3:
        return lane
    ...
    extrap_x = np.polyfit(
        lane[:, 1],
        lane[:, 0],
        deg=min(2, lane.shape[0] - 1))
    extrap_xs = np.polyval(extrap_x, lane[:, 1])
    diff_x = extrap_xs - lane[:, 0]

    extrap_z = np.polyfit(
        lane[:, 1],
        lane[:, 2],
        deg=min(2, lane.shape[0] - 1))
    extrap_zs = np.polyval(extrap_z, lane[:, 1])
    diff_z = extrap_zs - lane[:, 2]

    mask = np.logical_or(
        np.abs(diff_x) > max_dx,
        np.abs(diff_z) > max_dz)
    return lane[~mask]

    # mask = np.ones((lane.shape[0], 1))
    # last_idx = 0
    # for i, xyz in enumerate(lane):
    #     if i == 0:
    #         continue
    #     delta_x = lane[last_idx][0] - xyz[0]
    #     delta_y = lane[last_idx][1] - xyz[1]
    #     delta_z = lane[last_idx][2] - xyz[2]

    #     if np.abs(delta_x) > max_dx or np.abs(delta_z) > max_dz:
    #         if last_idx == 0:
...
def projective_transformation(Matrix, x, y, z):
    ...
    """
    ones = np.ones((1, len(z)), dtype=np.float32)
