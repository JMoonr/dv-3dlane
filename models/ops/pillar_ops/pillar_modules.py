...
class PillarMaxPooling(nn.Module):
  ...
  def forward(self, pts_xy, pts_batch_cnt, pts_feature, xyzs):
    """
    Args:
      pts_xy: (N1+N2..., 3)
      pts_batch_cnt: (N1, N2, ...)
    ...
    pillar_features = scatter_max(
        point_pillar_features,
        point_pillar_indices,
        pillar_indices.shape[0]) # (M, C)

    pillar_xyz = scatter_mean(
        torch.cat(xyzs, dim=0),
        point_pillar_indices.long(),
        0,
        torch.zeros(
            (pillar_indices.shape[0], 3),
            dtype=torch.float32,
            device=pillar_indices.device),
    )

    return spconv.SparseConvTensor(
        pillar_features, pillar_indices, 
        (self.height, self.width), batch), pillar_xyz
