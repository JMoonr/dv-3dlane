...
def bev_spatial_shape(pillar_size, point_cloud_range):
    W = round((point_cloud_range[3] - point_cloud_range[0]) / pillar_size[0])
    H = round((point_cloud_range[4] - point_cloud_range[1]) / pillar_size[1])
    return int(H), int(W)
...
class PillarQueryAndGroup(nn.Module):
    def __init__(self, pillar_size, point_cloud_range):
        ...
        pillar_size = pillar_size if isinstance(pillar_size, (tuple, list)) else [pillar_size for _ in range(2)]
        self.pillar_size = pillar_size
        
        self.spatial_shape = bev_spatial_shape(pillar_size, point_cloud_range)
        self.x_offset = pillar_size[0] / 2. + point_cloud_range[0]
        self.y_offset = pillar_size[1] / 2. + point_cloud_range[1]
    ...
    def forward(self, pts_xy, pts_batch_cnt, pts_features):
        ...
        pillar_centers = torch.stack((pillar_indices[:, 2] * self.pillar_size[0] + self.x_offset, 
                                      pillar_indices[:, 1] * self.pillar_size[1] + self.y_offset), dim=1)
